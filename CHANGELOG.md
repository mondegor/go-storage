...
## 2025-09-28 (v0.17.0)
### Added
- Добавлен `mrpostgres.ProcessWaitForNotification` для прослушивания и обработки событий поступающих из БД (через `LISTEN`);
...
- Убрана зависимость от библиотеки `go-webcore`;
- Доработан `mrpostgres.ConnAdapter`

## 2024-11-24
### Changed
- Некоторые ошибки заменены на `mrcore.ErrInternalWithDetails`;
- Обновлены зависимости библиотеки;

## 2024-11-17
### Added
- Добавлен интерфейс `mrstorage.SequenceGenerator`, а пакет `db.SequenceGenerator` перенесён в `sequence.Generator`;
- Добавлено ограничение по максимальному количеству выводимых параметров в `sql.logger.QueryTracer`;
- Добавлен `placeholdedvalues.SQL` для формирования повторяющиеся последовательностей, например,
  используемых в INSERT запросах при множественной вставке;

## 2024-11-05
### Added
- Библиотека формирования SQL запросов перенесена в `mrpostgres/builder` пакет,
  доработана, добавлен более дружелюбный интерфейс для использования;
- Для `Postgres` добавлена обработка ошибки `unexpected EOF`;
- Добавлен `logger.QueryTracer` для логирования SQL запросов;
- Добавлена структура `mrsql.DBTableInfo`;
- Добавлена функция `mrsql.ParseEntity()`, которая парсит структуру с тегами и возвращает `mrsql.EntityMeta`;
- Добавлен `db.SequenceGenerator` для генерации как одного ID, так и указанного кол-ва (на основе postgres последовательностей);

### Changed
- Поправлены примеры использования компонентов `builder.SQL`;

### Fixed
- Поправлена обработка ошибок в адаптере `mrpostgres.ConnAdapter`;

### Removed
- Удалён `mrstorage.MetaGetter` вместо него теперь используется `mrsql.DBTableInfo`;
